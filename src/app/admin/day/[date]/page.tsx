...
export default function AdminDayPage() {
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-[var(--muted)]">Read-only overview</p>
      <ul className="mt-4 space-y-2">
        {sessions === null ? (
          <li className="text-sm text-[var(--muted)]">Loading…</li>
        ) : sessions.length === 0 ? (
          <li className="text-sm text-[var(--muted)]">No sessions.</li>
        ) : (
          sessions.map((s) => (
            <li key={s.id}>
              <Link
                href={`/admin/day/${date}/session/${s.id}`}
                className="flex min-h-14 items-center justify-between rounded-xl border border-[var(--border)] bg-[var(--surface)] px-4 py-3"
              >
