...
export async function POST(req: NextRequest, ctx: Ctx) {
  ...
  const existingSet = new Set((existingRecords ?? []).map((r) => r.member_id as string));
  const toAppeal = uniqueMemberIds.filter((id) => !existingSet.has(id));
  if (toAppeal.length === 0) {
    return NextResponse.json({ error: "Selected members are already in attendance." }, { status: 400 });
  }
